...
describe 'nblog' do
  ...
  context 'user' do
    ...
    it 'should publish a post with multiple links and more' do
      text = 'I really love [this site](http://example.com), especially when '\
             'it comes _to [this](http://example.com/LOL)_.'
    ...
    it 'should see the RSS feed title with the HTML tags stripped' do
      ...
      expect(session.find(:xpath, '//item[1]/title')).to have_text(
        'I really love this site, especially when it comes to this.')
    end
    ...
    it 'should view a post' do
      ...
      expect(session.current_url).to match(%r{/p/\d+$})
